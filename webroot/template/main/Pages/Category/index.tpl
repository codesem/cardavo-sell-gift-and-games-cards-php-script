		...
		<div class="container">
			<div class="review-search-slider-section row justify-content-center">
                <div class="col-12 col-lg-12">
                    <h1 class="review-slider-title">{$cat_name}</h1>
                    <p class="review-slider-subtitle"><span class="badge badge-secondary">{$catnumber} {$_PRODUCTS}</span></p>
                </div>
                        ...
                        <div class="col-12">
                            <div class="filter row">
                                <div class="col-8 col-md-3">
                                    <h6 class="result">{$_SHOWING_ALL} <span class="badge badge-secondary">{$catnumber}</span> {$_RESULTS}</h6>
                                </div>
                    ...
                    <!-- =========================
                        ...
                        <div class="card category category-md">
                            <div class="card-header">
                                      <h5 class="title"><i class="fa fa-bars"></i> {$_CATEGORIES}</h5>
                            </div>
                        <div class="list-group">
            {foreach from=$categories item=foo}
                            <a href="{$CAT}{$foo['link']}" class="list-group-item list-group-item-action {if $cat_link == $foo['link']}active{/if}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {$foo['name']} {if $cat_link == $foo['link']}{/if} <span class="badge badge-secondary">{$foo['count']}</span></a>
            {if $cat_link == {$foo['slug']} || $parent_link == {$foo['slug']}}
														
                            <a href="{$CAT}{$foo['link']}" class="list-group-item list-group-item-action {if $cat_link == $foo['link']}active{/if}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {$foo['name']}</a>

	        {foreach from=$foo['subcategories'] item=subcat}
	        		{if $foo['slug'] != 'all'}
                                    <li><a href="{$CAT}{$foo['link']}/{$subcat['link']}" {if $cat_link == $subcat['link']} class="fw-bold"{/if}><i class="fa fa-angle-right" aria-hidden="true"></i> {$subcat['name']}</a></li>
					{/if}
	        {/foreach}
